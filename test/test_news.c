...
static void test_wrap(void) {
    char lines[2][128];
    int one = gfx_text_width(&FONT_PX11, "Wisconsin congressman");
    int n = gfx_wrap(&FONT_PX11, "Wisconsin congressman survives emergency plane landing, swims to safety", one + 2, lines, 2);
    CHECK(n == 2);
    CHECK(strcmp(lines[0], "Wisconsin congressman") == 0);
    CHECK(strlen(lines[1]) > 3 && strcmp(lines[1] + strlen(lines[1]) - 3, "...") == 0);
    CHECK(gfx_text_width(&FONT_PX11, lines[1]) <= one + 2);

    n = gfx_wrap(&FONT_PX11, "Short headline", 300, lines, 2);
    CHECK(n == 1 && strcmp(lines[0], "Short headline") == 0);

    // A word wider than the line is cut rather than lost.
    n = gfx_wrap(&FONT_PX11, "Supercalifragilistic", 30, lines, 2);
    CHECK(n == 2);
    CHECK(gfx_text_width(&FONT_PX11, lines[0]) <= 30 && strlen(lines[0]) > 0);

    CHECK(gfx_wrap(&FONT_PX11, "", 100, lines, 2) == 0);
    CHECK(gfx_wrap(&FONT_PX11, "   ", 100, lines, 2) == 0);
}
